...
class BodyPartsScreen extends StatelessWidget {
      ...
      builder: (context, constraints) {
        return Scaffold(
          appBar: CustomAppBar(
            ...
            actionsWidgets: [
              StreamBuilder(
                stream: controller.getBodypartsQuizProgress(),
                ...
                child: Column(
                  ...
                  children: [
                    Obx(() {
                      ...
                      final totalItems =
                          bodyparts.length + (bodyparts.length ~/ quizInterval);
                      return GridView.builder(
                        ...
                        itemBuilder: (context, index) {
                          ...
                          final color = bodyparts[colorIndex];
                          return GestureDetector(
                            onTap: () {
                              controller.setIndex(colorIndex);
                              Get.to(() => BodypartsDetailScreen());
                            },
                            child: Container(
  ...
  Widget buildQuizTile(int quizIndex) {
    return GestureDetector(
      onTap: () {
        final allbodyparts = controller.bodyparts;
        final groupSize = 4;
        final start = quizIndex * groupSize;
        if (start >= allbodyparts.length) return;
        final end = min(start + groupSize, allbodyparts.length);
        final group = allbodyparts.sublist(start, end);
        final questions = group.map((bodyparts) {
          final otherOptions = [...group]..remove(bodyparts)..shuffle();
          final options = ([bodyparts.name] + otherOptions.take(3).map((e) => e.name).toList())..shuffle();

          return QuestionModel(
              question: "What is the name of this body part?",
              options: options,
              correctAnswer: bodyparts.name,
              displayImage: bodyparts.imagePath,
